...
class CombinMap:
    ...
    def getCorrectionUnorientedEdges(self, ignoreIndices: int = math.inf) -> List[Tuple[int, int]]:
        allEdges = self.root.getAllBrins()
        allPreviousEdgesSource = [e.getAffectedSource() for e in allEdges]
        self.affectNodes()
        allNewEdgesSource = [e.getAffectedSource() for e in allEdges]
        ...
        while len(unseenBrins) > 0:
            ...
            for brin in face:
                ...
                if brin.beta2 in unseenBrins:
                    unseenBrins.remove(brin.beta2)
            for edge in face:
                source, dest = edge.getAffectedSource(), edge.getAffectedDest()
                if (source, dest) in unorientedEdges or (dest, source) in unorientedEdges:
                    continue
                unorientedEdges.append((source, dest))
    ...
    def affectNodes(self) -> List[int]:
        allEdges = self.root.getAllBrins()
        nodeIndex = max([e.getAffectedSource() for e in allEdges] + [e.getAffectedDest() for e in allEdges]) + 1
        # nodeIndex = -1
        for e in allEdges:
            if e.getAffectedSource() == -1:
                e.affectSource(nodeIndex)
                nodeIndex += 1
        return list(range(nodeIndex))
    ...
    def debug(self, labels: Union[None, List, Callable, bool] = None, tofile: Union[bool, str] = False, block: bool = True):
        plt.clf()
