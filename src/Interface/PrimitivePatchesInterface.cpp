...
//        std::sort(sqrDistToEachPasses.at(pos).begin(), sqrDistToEachPasses.at(pos).end());
    ...
    Matrix3<float> sqrDistToReef(voxelGrid->getDimensions(), 100000);
    for (size_t i = 0; i < availablePositions.size(); i++) {
        const Vector3& pos = availablePositions[i];

        for (size_t iReef = 0; iReef < transformedReefCurves.size(); iReef++) {
            sqrDistToReef.at(pos) = std::min(sqrDistToReef.at(pos), transformedReefCurves[iReef].estimateSqrDistanceFrom(pos));
        }
    }
...
//    avgPassesDirections /= 3.f;
    Vector3 avgPassesDirections = transformedReefCurves[0].getDirection(transformedReefCurves[0].estimateClosestTime(bestFitPos));
