...
void TerrainSavingInterface::display()
{
    // Nothing to display
}
...
void TerrainSavingInterface::saveTerrainGeometry(std::string filename)
{
    ...
    if (this->saveHeightmap) {
        ...
        file.open(filename + "-heightmap" + ".obj");
        file << m.toOBJ();
        file.close();
...
void TerrainSavingInterface::quickSaveAt(std::string folderName, std::string filePrefix, bool heightmap, bool voxels, bool layers)
{
    ...
    if (heightmap) {
        start = std::chrono::system_clock::now();
        m = this->heightmap->getGeometry();
        file.open(folderName + "/" + filePrefix + "-heightmap" + ".obj");
        file << m.toOBJ();
        file.close();
        end = std::chrono::system_clock::now();
    }
    ...
    if (voxels) {
        start = std::chrono::system_clock::now();
        m = this->voxelGrid->getGeometry();
        file.open(folderName + "/" + filePrefix + "-voxels" + ".obj");
        file << m.toOBJ();
        file.close();
        end = std::chrono::system_clock::now();
    }
    ...
    if (layers) {
        start = std::chrono::system_clock::now();
        m = this->layerGrid->getGeometry();
        file.open(folderName + "/" + filePrefix + "-layers" + ".obj");
        file << m.toOBJ();
        file.close();
        end = std::chrono::system_clock::now();
    }
