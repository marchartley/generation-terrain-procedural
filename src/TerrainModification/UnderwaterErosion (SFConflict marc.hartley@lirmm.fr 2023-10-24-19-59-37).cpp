...
Vector3 getSettlingVelocity(float particleDensity, float particleSize, float environmentDensity, float gravity, float capacity, float maxCapacity, Vector3& gravityDirection) {
    ...
    float capacityHinderingExponent = 5.f;
//                acceleration += std::clamp(constant * r * r * (matterDensity - environmentDensity) * gravity * (std::pow(capacity/maxCapacity, capacityHinderingExponent)), -1.f, 1.f);
    Vector3 acceleration = gravityDirection * (constant * r * r * (particleDensity - environmentDensity) * gravity * (1.f - std::pow(capacity/maxCapacity, capacityHinderingExponent))) * (capacity/maxCapacity);
    return acceleration;
}
...
//    posAndDirs[0] = {Vector3(12, 34, 5), Vector3()};
    ...
    for (int i = 0; i < this->rockAmount; i++)
    {
        ...
        initializeParticle(particle, initialPos, initialDir, .5f, matterDensity, capacity, maxCapacityFactor);//maxCapacity);
...
//            if (!rolling)
            Vector3 settling = Vector3();// getSettlingVelocity(particle.density, particle.radius, environmentDensity, gravity, particle.capacity, particle.maxCapacity, gravityDefault);
//            std::cout << settling << std::endl;
            particle.dir += (flowfield * flowfieldInfluence + settling) * dt;
