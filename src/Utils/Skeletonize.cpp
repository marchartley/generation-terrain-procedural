...
skeleton_tracer_t::polyline_t *skeleton_tracer_t::new_polyline(){
...
std::string skeleton_tracer_t::print_polyline(polyline_t *q){
    std::string str = "";
    if (!q){
        return str;
    }
...
std::string skeleton_tracer_t::print_polylines(polyline_t *q){
    std::string str = "";
    if (!q){
        return str;
    }
...
void skeleton_tracer_t::destroy_polylines(polyline_t *q){
    if (!q){
        return;
    }
    polyline_t* it = q;
...
void skeleton_tracer_t::reverse_polyline(polyline_t *q){
    if (!q || (q->size < 2)){
        return;
    }
    q->tail->next = q->head;
...
void skeleton_tracer_t::cat_tail_polyline(polyline_t *q0, polyline_t *q1){
    if (!q1){
        return;
    }
    if (!q0){
...
void skeleton_tracer_t::cat_head_polyline(polyline_t *q0, polyline_t *q1){
    if (!q1){
        return;
    }
    if (!q0){
...
void skeleton_tracer_t::add_point_to_polyline(polyline_t *q, int x, int y){
...
skeleton_tracer_t::polyline_t *skeleton_tracer_t::prepend_polyline(polyline_t *q0, polyline_t *q1){
    if (!q0){
        return q1;
    }
    q1->next = q0;
...
std::string skeleton_tracer_t::print_rects(){
    std::string str;
...
void skeleton_tracer_t::destroy_rects(){
    rect_t* it = rects.head;
    while(it){
        rect_t* jt = it->next;
        free(it);
...
void skeleton_tracer_t::add_rect(int x, int y, int w, int h){
#if SAVE_RECTS
...
bool skeleton_tracer_t::thinning_zs_iteration(int iter) {
    bool diff = false;
    for (int i = 1; i < H-1; i++){
        for (int j = 1; j < W-1; j++){
            int p2 = im[(i-1)*W+j]   & 1;
...
void skeleton_tracer_t::thinning_zs(){
    bool diff = true;
    do {
...
int skeleton_tracer_t::not_empty(int x, int y, int w, int h){
    for (int i = y; i < y+h; i++){
        for (int j = x; j < x+w; j++){
            if (im[i*W+j]){
                return 1;
...
int skeleton_tracer_t::merge_impl(polyline_t *c0, polyline_t *c1i, int sx, int isv, int mode){
...
skeleton_tracer_t::polyline_t *skeleton_tracer_t::merge_frags(polyline_t *c0, polyline_t *c1, int sx, int dr){
    if (!c0){
        return c1;
    }
    if (!c1){
...
skeleton_tracer_t::polyline_t *skeleton_tracer_t::chunk_to_frags(int x, int y, int w, int h){
...
skeleton_tracer_t::polyline_t *skeleton_tracer_t::trace_skeleton(int x, int y, int w, int h, int iter){
    // printf("_%d %d %d %d %d\n",x,y,w,h,iter);
...
void skeleton_tracer_t::print_bitmap(){
    for (int i = 0; i < H; i++){
        for (int j = 0; j < W; j++){
            printf("%d",im[i*W+j]);
        }
...
char *skeleton_tracer_t::trace(char *img, int w, int h){
...
void skeleton_tracer_t::destroy(){
    if (im){
        free(im);
    }
    destroy_rects();
