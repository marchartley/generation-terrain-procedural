...
static void check_program_errors(GLuint program_id) {
    GLint is_ok = 0;
    GlobalsGL::f()->glGetProgramiv(program_id, GL_LINK_STATUS, &is_ok);
    if (!is_ok) {
        GLint max_length = 0;
        GlobalsGL::f()->glGetProgramiv(program_id, GL_INFO_LOG_LENGTH, &max_length);

        // The maxLength includes the NULL character
        std::vector<GLchar> info_log(max_length);
        GlobalsGL::f()->glGetProgramInfoLog(program_id, max_length, NULL, &info_log[0]);
