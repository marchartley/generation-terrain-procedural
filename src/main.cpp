...
#include "FastWFC/external/rapidxml.hpp"
#include "FastWFC/image.hpp"
#include "FastWFC/rapidxml_utils.hpp"
#include "FastWFC/utils.hpp"
...
//  unsigned height = stoi(rapidxml::get_attribute(node, "height", "48"));
  ...
  const std::string image_path = "C:/codes/Qt/generation-terrain-procedural/src/FastWFC/samples/" + name + ".png";
  ...
  for (unsigned i = 0; i < screenshots; i++) {
    for (unsigned test = 0; test < 10; test++) {
      ...
      if (success.has_value()) {
        write_image_png("C:/codes/Qt/generation-terrain-procedural/src/FastWFC/results/" + name + to_string(i) + ".png", *success);
...
void read_simpletiled_instance(xml_node<> *node,
  ...
  ifstream config_file("C:/codes/Qt/generation-terrain-procedural/src/FastWFC/samples/" + name + "/data.xml");
  ...
  for (unsigned test = 0; test < 10; test++) {
    ...
    std::optional<Array2D<Color>> success = wfc.run();
    if (success.has_value()) {
      write_image_png("C:/codes/Qt/generation-terrain-procedural/src/FastWFC/results/" + name + "_" + subset + ".png", *success);
      cout << name << " finished!" << endl;
      break;
    } else {
      cout << "failed!" << endl;
...
void tryFastWFC() {
  ...
  read_config_file("C:/codes/Qt/generation-terrain-procedural/src/FastWFC/samples.xml");
