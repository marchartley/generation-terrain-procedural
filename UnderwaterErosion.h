...
//    std::vector<std::vector<Vector3>> Apply(int avoidMatter = -1);
    std::tuple<std::vector<std::vector<Vector3>>, std::vector<std::vector<Vector3>>> Apply(Vector3* startingPoint = nullptr, Vector3* originalDirection = nullptr, int avoidMatter = 10, float flowfieldFactor = 0.1, bool returnEvenLostRocks = false);
